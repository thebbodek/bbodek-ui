...
export const CalendarComponentDayText = ({
  ...
  const isActiveDate = isStartDate || isEndDate || isSingleSelectedDate;
  const isMarkedDate = isActiveDate || calendarDate.isToday || isExceptionDate;
  const disabledTextColor = !isActiveDate ? 'text-gray-05' : 'text-white';
  const defaultTextColor =
    isMarkedDate || isTemporaryDate ? 'text-white' : 'text-black';
    ...
    <div
      className={clsx('relative flex flex-col items-center justify-between', {
        "before:bg-primary-00 before:absolute before:z-20 before:block before:h-full before:w-1/2 before:translate-x-1/2 before:content-['']":
          isStartDate || isEndDate || (afterAllDate && isSingleSelectedDate),
    ...
    >
      <div
        className={clsx(
          ...
          !isExceptionDate && disabled ? disabledTextColor : defaultTextColor,
